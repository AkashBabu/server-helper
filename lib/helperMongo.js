...
helperMongo.validateNonExistence = function (collName, validate, cb) {
    db.collection(collName).findOne(validate, function(err, result){
        cb(err, !result);
    });
}
...
helperMongo.validateId = function (collName, id, convertId, cb) {
...
helperMongo.getForId = function (collName, id, convertId, cb) {
...
helperMongo.update = function (collName, obj, convertId, cb) {
    var query = {};
...
helperMongo.getList = function (collName, obj, cb) {
    ...
    if(obj.search){
        ...
        obj.query[obj.search_field || 'name'] = {
            $regex: regex
        }
    }
    db.collection(collName).find(obj.query, obj.project || {})
        .sort(obj.sort || {})
        .skip(skip)
        .limit(limit, cb);
}
