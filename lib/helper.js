                ...
                if (obj[fieldSpec.name] != undefined || obj[fieldSpec.name] != null) {
                    ...
                    if (fieldSpec.type.constructor == Array) {
                        if (fieldSpec.type.indexOf(obj[fieldSpec.name].constructor) == -1) {
                            logger.log('Invalid type of:', fieldSpec.name);
                            cb(errMsg1, false);
                        }
                                ...
                                function (validate, cb1) {
                                    ...
                                    if (validate.length > argsLen) {
                                        function callback(err, result) {
                                            var self1 = this;
                                            if (err) {
                                                logger.log("Validation Error ", self1.name)
                                                cb1(err, false);
                                            } else {
                                                if (result) {
                                                    cb1(null, true);
                                                } else {
                                                    logger.log("Validation Failed ", self1.name)
                                                    cb1(errMsg, false);
                                                }
                                            }
                                        }
                                        ...
                                        if (!validate.apply(null, fieldSpec.validateArgs)) {
                                            logger.log('Validation Failed ', fieldSpec.name);
                                            cb1(errMsg, false);
                                        } else {
                                            cb1(null, true);
                                        }
                    ...
                    logger.log('Field ', fieldSpec.name, " not present");
                    cb(errMsg1, false);
                }
            }, callback);
    }
